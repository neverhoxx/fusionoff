...
export function ProductCard({ item }: { item: Product }) {
    ...
    return (
        <Link href={`/product/hoodies/${item.id}`} className="md:w-[352px] flex flex-col mt-[25px] select-none w-[155px] cursor-pointer">
            <div className="relative group">
                ...
                {item.images.length > 1 && (
                    <>
                        <button
                            onClick={prevImg}
                            className="absolute top-1/2 left-2 -translate-y-1/2 text-black p-2 rounded-full opacity-0 group-hover:opacity-100 transition cursor-pointer"
                        >
                            <IoIosArrowBack />
                        </button>
                        <button
                            onClick={nextImg}
                            className="absolute top-1/2 right-2 -translate-y-1/2 text-black p-2 rounded-full opacity-0 group-hover:opacity-100 transition cursor-pointer"
                        >
                            <IoIosArrowForward />
                        </button>
                    </>
